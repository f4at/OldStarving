...
class AmethystHelmetMod extends Mod {
    ...
    on(event: ModEvent, data: any) {
        switch (event) {
            ...
            case "sprite":
                data.SPRITE[NAME] = ID;
                data.sprite[data.SPRITE[NAME]] = [];
                data.sprite[data.SPRITE[NAME]][data.SPRITE.DAY] = this.client.CTI(create_amethyst_helmet(this.client, .78, true, "#0d1b1c #8c55b7 #652d8e #79359e #5eccd1 #4badad #9e6ac6".split(" ")));
                data.sprite[data.SPRITE[NAME]][data.SPRITE.NIGHT] = this.client.CTI(create_amethyst_helmet(this.client, .78, true, "#0d1b1c #4e4e93 #393977 #444187 #2b9390 #277a74 #7da8db".split(" ")));
                break;
            case "draw_clothe":
                if (data.id === this.client.SPRITE[NAME]) {
                    this.client.draw_image_transition(data.sprite, -data.sprite.width / 2 + 1.5 * this.client.scale, -data.sprite.height / 2 + 1 * this.client.scale);
                }
                break;
        }
    }
...
function create_amethyst_helmet(client, c, g, f) {
    ...
    d.closePath();
    client.fill_path(d, f[4], f[5], 4 * c);
    d.beginPath();
    d.scale(1.1, 1.1);
    d.lineCap = "round";
    d.lineJoin = "round";
    ...
    d.rotate(0);
    client.round_rect(d, -61 * c, -38.5 * c, 122 * c, 40 * c, 20 * c);
    d.restore();
    client.fill_path(d, f[1], f[2], 4 * c);
    d.save();
    d.translate(100 * c, 71.36874389648438 * c);
    d.rotate(0);
    client.round_rect(d, -60.5 * c, -18 * c, 121 * c, 30 * c, 20 * c);
    d.restore();
    client.fill_path(d, f[3], f[2], 4 * c);
    d.beginPath();
    d.lineCap = "round";
    d.lineJoin = "round";
    d.moveTo(144 * c, 80 * c);
    ...
    d.bezierCurveTo(86 * c, 62 * c, 86 * c, 73 * c, 86 * c, 80 * c);
    d.restore();
    client.fill_path(d, f[6], f[2], 4 * c);
    return g;
}
